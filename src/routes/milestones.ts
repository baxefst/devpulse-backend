...
msRoute.post('/drops/:dropId/milestones', authMiddleware, apiRateLimit,
...
msRoute.patch('/milestones/:id', authMiddleware, apiRateLimit, async (c) => {
  ...
  if (ms.user_id !== sub) return c.json({ error: 'Forbidden' }, 403)
...
msRoute.post('/milestones/:id/proof', authMiddleware, proofRateLimit,
  zValidator('json', submitProofSchema), async (c) => {
  const { sub } = c.get('user')
  const id      = c.req.param('id')
  ...
  if (existing) return c.json({ error: 'Proof already submitted for this milestone' }, 409)
