...
lb.get('/', zValidator('query', leaderboardQuerySchema), async (c) => {
    ...
    try {
        const allUsers = await db.select({
            ...
            streak: users.streak,
            rank: users.rank,
            reputation_score: sql `CAST(${users.reputation_score} AS FLOAT)`,
        })
            .from(users)
            .orderBy(desc(users.reputation_score), desc(users.streak), asc(users.username))
            .limit(limit)
            .offset(offset);
        ...
        const enriched = await Promise.all(filtered.map(async (u) => {
            ...
            const cr = ms.length > 0
                ? Math.round(ms.reduce((s, m) => s + (weights[m.status] ?? 0), 0) / ms.length)
                : 0;
            return {
                ...u,
